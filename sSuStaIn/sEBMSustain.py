        ...
        if "save_path" not in kwargs:
            ...
            if len(figs) > 1:
                ...
                for num_subtype, fig in zip(range(N_subtypes), figs):
                    ...
                    plot_fname = Path(
                        self.output_folder
                    ) / f"{self.dataset_name}_subtype{N_subtypes - 1}_subtype{num_subtype}-separated_heatmap_matrix_{N_folds}fold_CV"
                    # Save the figure
            ...
            # Otherwise default single plot
                ...
                plot_fname = Path(
                    self.output_folder
                ) / f"{self.dataset_name}_subtype{N_subtypes - 1}_heatmap_matrix_{N_folds}fold_CV"
                # Save the figure
                fig.savefig(plot_fname, bbox_inches='tight')
                fig.show()
    ...
    def run_sustain_algorithm(self, plot=False, plot_format="png", **kwargs):
        ...
        for s in range(self.N_S_max):
            ...
            if plot:
                ...
                base_width                      = 25
                height_per_biomarker            = 0.25 # enough space
                num_biomarkers                  = self.__sustainData.getNumBiomarkers()

                dynamic_height = max(20, num_biomarkers * height_per_biomarker) # Ensure a minimum height to have adequate space per feature

                tsv_rows = [("Biomarker", "Subtype", "Assigned Stage")]

                # For each subtype, iterate through all biomarkers
                for subtype_idx, subtype in enumerate(self._plot_subtype_order):
                    subtype_sequence = ml_sequence_EM[subtype]  # List/array of biomarker indices
                    for stage_idx, biomarker_array in subtype_sequence.items(): # Loop through each stage and corresponding list of biomarkers
                        for biomarker_idx in biomarker_array:
                            biomarker_label_idx = self._plot_biomarker_order[biomarker_idx]
                            biomarker_name = self.biomarker_labels[biomarker_label_idx]
                            tsv_rows.append((biomarker_name, str(subtype_idx + 1), str(stage_idx + 1)))

                # Save TSV to file
                tsv_path = Path(self.output_folder) / f"{self.dataset_name}_subtype{s+1}_biomarker_inferred_stages.tsv"
                ...
                figs, ax = self._plot_sustain_model(ml_sequence_EM=ml_sequence_EM,
                    ...
                    save_path=Path(self.output_folder) / f"{self.dataset_name}_subtype{s+1}_heatmap_matrix",
                    figsize=(base_width, dynamic_height),
                    **kwargs
                )
                for fig in figs:
                    fig.show()
