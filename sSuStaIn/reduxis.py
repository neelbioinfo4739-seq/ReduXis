...
def read_flexible_csv(filepath, index_col=None, **kwargs):
    ...
    # Smart read
    df = pd.read_csv(filepath, sep=delimiter, index_col=index_col, **kwargs)

    # If no index_col was passed but there's an 'index' column, set it as index
    if index_col is None and 'index' in df.columns:
...
def check_missing_values(df: pd.DataFrame, name="DataFrame", min_fraction_passed=0.70):
    threshold = 0.2
    n_cols = len(df.columns)

    # Compute missingness per subject (row-wise)
    row_missing_fraction = df.isna().sum(axis=1) / n_cols
    bad_rows = df.index[row_missing_fraction > threshold]

    if not bad_rows.empty:
        preview_subjects = bad_rows[:10].tolist()
        logging.warning(
            f"{len(bad_rows)} subjects (rows) exceed the missing data threshold of {threshold * 100:.0f}%."
            f" Consider exclusion or targeted imputation. "
            f"Example subject indices: {preview_subjects}..."
        )

    # Drop bad subjects
    df.drop(index=bad_rows, inplace=True)

    # Post-drop sanity check
    n_total = len(bad_rows) + len(df)
    n_retained = len(df)

    if n_retained / n_total < min_fraction_passed:
        logging.error(
            f"Too few subjects remain in {name} after filtering ({n_retained}/{n_total}) — exiting."
        )
        sys.exit(1)

    logging.info(f"✓ Retained {n_retained}/{n_total} subjects in {name} after missing value filtering")
    return df
...
def check_numeric_range(df: pd.DataFrame, iqr_multiplier=1.5, name="DataFrame"):
    ...
    for col in df.columns:
        ...
        neg_ratio = num_negative / len(series)

        # Determine lower bound
        lower = q1 - iqr_multiplier * iqr if neg_ratio > 0.05 else minv
        ...
        if outliers.any():
            df_cleaned[col] = series.clip(lower=lower, upper=upper)
...
def run_feature_selection(X: pd.DataFrame, labels: pd.DataFrame, valid_outcomes: list, output_dir: Path, min_features: int, max_features: int, run_selection: bool, min_stability_threshold=0.5):
    ...
    if not run_selection:
        ...
        # is X.columns.tolist or something the list of biomarkers if columns = biomarkers???
        return X, y, full_report, cm, ordinal_acc, final_stability_score, N_feats, X.columns.tolist(), N_feats
    ...
    for run in range(5): # 5-fold cross validation
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, stratify=y, random_state=(42+run)) # train-test split
        X_tr_os, y_tr_os = smote.fit_resample(X_train, y_train)
    ...
    print("")
    return X[selected], y, reduced_report, cm, ordinal_acc, final_stability_score, N_feats, selected, len(selected)
...
def run_sensitivity_analysis(X: pd.DataFrame, y: pd.DataFrame, valid_outcomes: list, output_dir: Path, min_features: int, max_features: int):
    ...
    for threshold in thresholds:
        ...
        _, _, _, _, _, _, _, _, selected_feature_count = run_feature_selection(X, y, valid_outcomes, output_dir, min_features, max_features, X.shape[1] > max_features, threshold)
...
def read_and_preprocess(data_f: Path, meta_f: Path, outcomes: list):
    ...
    # Load
    X = read_flexible_csv(data_f, index_col="SubjectID")
    ...
    # Map normalized inputs back to original Outcome values based on order of input
    valid_outcomes = []
    seen = set()

    for norm_val in normalized_input_outcomes:
        matched_vals = lab.loc[lab['_Outcome_norm'] == norm_val, 'Outcome'].unique()
        for val in matched_vals:
            if val not in seen:
                valid_outcomes.append(val)
                seen.add(val)
...
def plot_biomarker_stage_heatmap(X: pd.DataFrame, biomarkers: list, outcomes: list, assigned_df: pd.DataFrame, output_dir: Path):
    ...
    plt.savefig(output_path, dpi=300) # specify maximum width?
    plt.close()
...
# ──────────────────────────────────────────────────────────────────────────────
# List of Selected Biomarkers Helper
def save_selected_features(selected_features, final_features, output_dir, file_format='tsv'):
    os.makedirs(output_dir, exist_ok=True)
    col_name = f"Selected Biomarkers (N = {final_features})"
    df = pd.DataFrame(selected_features, columns=[col_name])
    file_path = os.path.join(output_dir, f"selected_features.{file_format}")

    if file_format == 'csv':
        df.to_csv(file_path, index=False)
    else:  # default to TSV
        df.to_csv(file_path, sep='\t', index=False)
    logging.info(f"Saved list of selected biomarkers.")

# ──────────────────────────────────────────────────────────────────────────────
# Chi-Squared Analysis, Nested and Stratified
def perform_chi_squared_analysis(assigned_df, output_dir, max_stages):
    # Clean and extract numeric stages
    stage_series = assigned_df['Assigned Stage'].dropna().astype(int)
    min_stage, max_stage = stage_series.min(), max_stages
    stages = list(range(min_stage, max_stage + 1))
    subtypes = sorted(assigned_df['Assigned Subtype'].dropna().unique())
    outcomes = assigned_df['Outcome'].dropna().unique()
    ...
    for st in subtypes:
        ...
        p_val    = min(chi2.sf(chi2_stat, df) * total_subtypes, 1.0) # Bonferroni correction for multiple subtype-level GOF tests (i.e. multiple comparisons)
...
def run_sustain_pipeline(args):
    ...
    # For instance, a chain of multiple kinases, phosphatases, transcription factors, all could be behind the 20 or so features that determine Stage 1 vs 2 of a disease
    print("")
    ...
    args.min_clust_size = min(max(args.min_clust_size, 5), 20)
    ...
    max_subtypes = min(5, n_subjects // (args.stages * 5))
    ...
    min_features = max_possible_stages * args.min_clust_size
    X_reduced, y, report, cm, ordinal_acc, final_stability_score, initial_features, selected_features, final_features = run_feature_selection(X_preprocessed, lab, valid_outcomes, Path(args.output_dir), min_features, args.max_features, do_selection, optimal_threshold)

    # Display preprocessed data, slow it down enough for a user to read and verify before letting software continue
    print("\n")
    logging.info("Displaying preprocessed data...")
    time.sleep(1)
    print(X_reduced)
    time.sleep(2)

    # Display metadata with subject disease outcomes as labels
    print("\n")
    logging.info("Displaying subject metadata with outcome labels...")
    time.sleep(1)
    print(lab)
    time.sleep(2)

    # Save necessary reports generated from feature selection and internal validation via classification
    ...
    save_selected_features(selected_features, final_features, args.output_dir)

    # Save the list selected_features as a TSV or CSV with one column, with title "Selected Biomarkers (N = {final_features})"
    ...
    # Step 2: Warn the user about sparse subtypes, but don't reorganize
    REQUIRED_SUBJECTS = 0.05 * n_subjects
    for subtype, count in subtype_counts.items():
        if count < REQUIRED_SUBJECTS:
            logging.warning(f"Subtype {subtype} has only {count} subjects, when at least {int(REQUIRED_SUBJECTS)} are recommended to avoid sparsity. Consider reducing the number of subtypes requested.")
    ...
    perform_chi_squared_analysis(assigned_df, Path(args.output_dir), args.stages)
...
def parse_args():
    ...
    parser.add_argument("--max-features", type=int, default=100, help="Maximum number of features for scaled EBM")
            ...
            if req in ("data_file", "metadata_file"):
                ...
                while True:
                    if req == "min_clust_size":
                        ...
                        value = interactive_prompt(prompt_name + default_str, use_tab_completion=False,
                            custom_prompt=f"{Fore.MAGENTA}Please enter the maximum number of features to hold in the model (default: {current_val}, max: 150):")
                        if value.isdigit():
                            setattr(args, attr, min(150, int(value)))
                            break
                    else:
                        value = interactive_prompt(prompt_name + default_str, use_tab_completion=False,
                    ...
                    if value.isdigit():
                        ...
                        logging.warning(f"'{value}' isn't a valid number. Try again.")
            elif req == "outcomes":
                while True:
                    logging
                    value = interactive_prompt(prompt_name + default_str, use_tab_completion=False,
